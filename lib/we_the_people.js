...
var WeThePeople = function(apiKey, opt_useSandbox, opt_maxRequestsPerMinute) {
  ...
  var maxRequestsPerMinute = lodash.isUndefined(opt_maxRequestsPerMinute) ? 1250 : opt_maxRequestsPerMinute;

  if (maxRequestsPerMinute >= 1500) {
    throw new Error('The "We the People" API supports a maximum of 1500 requests per minute from a single client');
  }

  this.requestProxy = new RequestProxy(apiKey, maxRequestsPerMinute);
